...
function App() {
  ...
  const fetchData = () => {
    fetch('http://localhost:3001/api/TestTable')
  ...
  const handleAdd = async () => {
    ...
    await fetch('http://localhost:3001/api/TestTable', {
      method: 'POST', 
  ...
  const handleUpdate = (id) => {
    fetch(`http://localhost:3001/api/TestTable/${id}`, {
        ...
        <tbody>
          {data.map(item => (
            <tr key={item.ID}>
              ...
              <td>
                <img
                  src={`http://localhost:3001/images/${item.ID}.jpg?${Date.now()}`}
                  alt={`${item.Name}の画像`}
                  style={{ width: '60px', height: '60px', objectFit: 'cover', display: 'block',border: '1px solid #ccc' }}
                  onError={(e) => {
                    e.target.onerror = null;
                    e.target.src = 'http://localhost:3001/images/placeholder.jpg';
                  }}
                />
              </td>
              <td>
